...
$sql1 = mysqli_query($conexion, "select * from users");
    ...
    ?>
											...
											<div class="col-xs-3 col-lg-2">
												<div class="input-group">
													<select id="categoria" class="form-control" onchange="load(1);">
														<option value="">Categoria</option>
														<option value="">Todos</option>
														<?php
$sql1 = mysqli_query($conexion, "SELECT id_linea, nombre_linea FROM lineas");
    while ($rw1 = mysqli_fetch_array($sql1)) {
        ?>
															<option value="<?php echo $rw1['id_linea'] ?>"><?php echo $rw1['nombre_linea']; ?></option>
															<?php
}
    ?>
													</select>
...
$('.daterange').daterangepicker({
	...
	function load(page){
		...
		var parametros = {"action":"ajax","page":page,'range':range,'estado_factura':estado_factura,'employee_id':employee_id, 'categoria': categoria};
		$("#loader").fadeIn('slow');
		$.ajax({
...
<script>
	function reporte(){
		...
		var employee_id=$("#employee_id").val();
		var categoria = $('#categoria').val();
		VentanaCentrada('../pdf2/rep_ventas.php?daterange='+daterange+"&estado_factura="+estado_factura+"&employee_id="+employee_id+"&categoria="+categoria,'Reporte','','800','600','true');
	}
