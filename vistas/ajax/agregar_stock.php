...
if (empty($_POST['quantity'])) {
    ...
    $subtotal = $quantity * $costo;
    $cant_saldo  = $rww['cant_saldo'] + $quantity;
    //$nueva_cantidad = $cant_saldo - $cantidad;
    //$nuevo_saldo    = $cant_saldo * $precio_venta;
    $saldo_total     = ($rww['total_saldo'] + $subtotal);
    $costo_promedio = ($rww['total_saldo'] + $subtotal) / $cant_saldo;
    $tip            = 3;
    guardar_entradas($fecha, $id_producto, $quantity, $costo, $subtotal, $cant_saldo, $costo_promedio, $saldo_total, $fecha, $user_id, $tip);
    if ($update) {
        $messages[] = "Stock ingresado correctamente.";
    } else {
