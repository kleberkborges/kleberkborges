...
if ($action == 'ajax') {
    ...
    if($categoria) {
        $sWhere .= " AND facturas_ventas.id_factura IN(SELECT detalle_fact.id_factura FROM detalle_fact_ventas detalle_fact, productos p WHERE p.id_linea_producto = '$categoria' AND detalle_fact.id_producto = p.id_producto GROUP BY detalle_fact.id_factura) ";
    }
...
$finales = 0;
        while ($row = mysqli_fetch_array($query)) {
            ...
            ?>
                    <tr>
                        <td class='text-center'><label class='badge badge-purple text-light'><?php echo $factura; ?></label></td>
                        <td><?php echo $cliente; ?></td>
                        <td class='text-center'><?php echo $fecha; ?></td>
                        <td class='text-center'><span class="badge <?php echo $label_class; ?>"><?php echo $text_estado; ?></span></td>
                        <td><?php echo $user_fullname; ?></td>
                        <td class='text-left'><b><?php echo $simbolo_moneda . '' . formatear_moneda($simbolo_moneda, $total); ?></b></td>
                    </tr>
                    <?php }?>
                </table>
            </div>
